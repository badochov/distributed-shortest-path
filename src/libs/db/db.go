...
func (d db) GetCoordsBounds(ctx context.Context, generation Generation) (CoordsBetween, error) {
	// TODO [badochov] This funcion should return right-open interval [a, b)
	// including coordinates of all the vertices

	v := d.q.Vertex
	min_latitude, err := d.q.WithContext(ctx).Vertex.Where(
		v.Generation.Eq(generation),
	).Select(v.Latitude.Min()).First()
	if err != nil {
		return CoordsBetween{}, err
	}

	max_latitude, err := d.q.WithContext(ctx).Vertex.Where(
		v.Generation.Eq(generation),
	).Select(v.Latitude.Max()).First()
	if err != nil {
		return CoordsBetween{}, err
	}

	min_longitude, err := d.q.WithContext(ctx).Vertex.Where(
		v.Generation.Eq(generation),
	).Select(v.Longitude.Min()).First()
	if err != nil {
		return CoordsBetween{}, err
	}

	max_longitude, err := d.q.WithContext(ctx).Vertex.Where(
		v.Generation.Eq(generation),
	).Select(v.Longitude.Min()).First()
	if err != nil {
		return CoordsBetween{}, err
	...
	return CoordsBetween{
		Latitude: MinMax{
			Min: min_latitude.Latitude,
			Max: max_latitude.Latitude,
		},
		Longitude: MinMax{
			Min: min_longitude.Longitude,
			Max: max_longitude.Longitude,
		},
	}, nil
}
